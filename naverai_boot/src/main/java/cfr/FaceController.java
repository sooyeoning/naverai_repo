...
public class FaceController {
	...
	public ModelAndView faceresult2(String image) { //jsp에서 사용하고 싶을때 : ${param.image}
		//사용자에게 전달받은 이미지명으로 서비스 클래스 요청 > naveropenapi > json으로 결과 전달
		String faceresult= service2.test(image);
		
		//서비스 결과값 전달받음
		ModelAndView mv = new ModelAndView();
		mv.addObject("faceresult2", faceresult);
		//mv.setViewName("faceresult2"); //성별 나이 감정 얼굴방향 눈코입
		mv.setViewName("faceresult3"); //canvas(이미지 도형) 얼굴사각형
		return mv;
	}
